    ...
    def reg() :
        if name.get() == '' :
            ...
            if password.get() == password2.get() :                
                passphrase = sha512(f"{password.get()}".encode('utf-32')).hexdigest()
                cur.execute(f"insert into users values('{name.get()}' , '{Email.get()}' , '{username.get()}' , '{passphrase}');")
                db.commit()
...
def login(root) :
    ...
    def LOGIN() :
        if username.get() == '' :
            ...
            passphrase = sha512(f"{password.get()}".encode('utf-32')).hexdigest()
            cur.execute(f"select * from users where username = '{username.get()}' and password = '{passphrase}';")
            L1 = cur.fetchall()
            if any(L1) :                
                root_login.destroy()
